...
export default function PatientAppointmentsPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-slate-900 dark:text-white mb-8">My Appointments</h1>
      <div className="glass rounded-2xl overflow-hidden">
        {Array.isArray(appointments) && appointments.length === 0 ? (
          <div className="p-12 text-center text-slate-500">No appointments yet. Book one from the home page.</div>
        ) : (
          <table className="w-full">
            ...
            <tbody>
              {(appointments as { id: string; date: string; startTime: string; status: string }[]).map((a) => (
